...
2. 创建或更新 `plan.md`。
3. 创建或更新 `tasks.md`。
4. 每个执行文档必须引用相关 Truth。`spec.md` 中每条 EARS 验收语句末尾必须标注其所验证的 `[RULE-...]` 标识，且每个被标注的标识必须出现在 Truth References 表中（RULE-GOV-011）。
5. task 必须可独立验收，并包含验收命令或步骤；该步骤必须标注其所验收的同一 `RULE-...` 标识（RULE-GOV-011）。
