...
### RULE-GOV-013: single_task Self-reporting Discipline
...
Task completion records in the single_task phase must explicitly state acceptance coverage scope and leave an auditable triage record after failures.

- Each task completion report and corresponding `tasks.md` task must declare `Acceptance scope:` with one of these values: `single`, `chain-midpoint`, or `chain-endpoint covering NNN.x-NNN.y`.
- Chain-merge decision rules, boundaries, and defaults are carried by R1 in `.p2t2c/prompts/05_execute_single_task_prompt.md`; they do not enter this Rule.
- If any Acceptance, build, test, lint, or governance check failure is retried or stopped, task Actual must record the triage label and retry count.
- Triage labels and handling are carried by R2/R3 in `.p2t2c/prompts/05_execute_single_task_prompt.md`; project-stack keywords and tool paths are extended by the project-owned `AGENTS.md` or equivalent entrypoint.

Rationale:

Acceptance merging reduces command invocations, but without recording the scope, Closure cannot tell whether a task was accepted on its own or covered by a chain endpoint. Failure triage labels and retry counts make environment failures, assertion failures, and crashes auditable while keeping project-stack keywords out of governance Truth.

Validation:

- Every task in `tasks.md` has `Acceptance scope:`.
- Any retried or stopped failure in task Actual contains one of `compile_error`, `unit_assertion`, `sandbox_environment`, or `runtime_crash`, plus a retry count.
- The phase map generated by `make check` includes `RULE-GOV-013` for `single_task` and `acceptance`.

Stop-the-line if:

- A task lacks `Acceptance scope:`.
- A failure is retried or stopped but Actual lacks the triage class or retry count.
- Implementation asks to put project-specific keywords or triage tool paths in this Rule.
