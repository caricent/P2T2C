...
usage() {
  cat <<'EOF'
Usage:
  scripts/p2t2c_install.sh --dry-run --target PATH
  scripts/p2t2c_install.sh --apply --target PATH
...
elif [[ -f "$target_root/scripts/check_p2t2c.sh" ]]; then
  if (cd "$target_root" && bash scripts/check_p2t2c.sh >/tmp/p2t2c-install-check.log 2>&1); then
    validation="bash scripts/check_p2t2c.sh: passed"
  else
    validation="bash scripts/check_p2t2c.sh: failed; see /tmp/p2t2c-install-check.log"
  fi
fi

{
  ...
  if [[ "${#conflicts[@]}" -eq 0 ]]; then
    ...
    echo "- Add P2T2C instructions to existing AGENTS.md if needed."
    echo "- Add P2T2C targets to existing Makefile if needed."
    echo "- Keep existing project README and link to P2T2C docs manually if needed."
  fi
