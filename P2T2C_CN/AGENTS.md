...
1. `AGENTS.md`
2. `project_config.yaml`；如果缺失，读取 `project_config.example.yaml`
5. 下方列出的阶段额外输入
...
| 任务 | Prompt | 文件写入 |
|---|---|---|
| 初始化仓库 | `prompts/01_bootstrap_repository_prompt.md` | 是，仅骨架 |
| 生成 Change Pack | `prompts/02_generate_change_pack_prompt.md` | 否 |
| 应用 Change Pack | `prompts/03_apply_change_pack_prompt.md` | 是，仅 Gate A 后 |
| 生成执行包 | `prompts/04_generate_execution_pack_prompt.md` | 是 |
| 执行单个任务 | `prompts/05_execute_single_task_prompt.md` | 是，仅一个任务 |
| 验收与收口 | `prompts/06_acceptance_and_closure_prompt.md` | 是，仅执行文档，除非 Truth Drift 暂停 |

阶段额外读取：

- Change Pack：当前 CP、相关 SoT、ADR、`templates/change_pack/CHANGE_PACK_TEMPLATE.md`
- Apply Change Pack：已批准 Change Pack、相关 SoT、ADR、truth templates、`docs/sot/manifest.yaml`
- Execution Pack：相关 CP、SoT、ADR、`sdd/templates/spec.md`、`sdd/templates/plan.md`、`sdd/templates/tasks.md`
...
- Install、upgrade：`README.md`、install 或 upgrade script、`.p2t2c/ownership.yaml`
...
- 不要把业务规则写入 `AGENTS.md`、`.agents/rules/`、prompt、测试、代码注释或导航文件。
