...
is_project_owned() {
  case "$1" in
    docs/sot/product/*|docs/sot/data/*|docs/sot/api/*|docs/sot/client/*|docs/sot/server/*|docs/sot/ai/*|docs/sot/testing/*)
      return 0
      ;;
    docs/adr/ADR-*.md|docs/submit_proposals/SP-*.md|docs/change_packs/CPK-*.md|docs/closure/CR-*.md|specs/*/*|src/*|tests/*|database/*|package.json)
      return 0
      ;;
    *)
      return 1
...
for migration in \
  ...
  "$source_root/.p2t2c/migrations/0.11.0-to-0.12.0.md"
do
...
mkdir -p "$upgrade_dir/backup/.p2t2c"
cp "$target_root/.p2t2c/lock.sha256" "$upgrade_dir/backup/.p2t2c/lock.sha256"
...
if [[ -f "$target_root/.p2t2c/bin/check_p2t2c.sh" ]]; then
  if (cd "$target_root" && bash .p2t2c/bin/check_p2t2c.sh >/tmp/p2t2c-upgrade-check.log 2>&1); then
    validation="bash .p2t2c/bin/check_p2t2c.sh: passed"
  else
    validation="bash .p2t2c/bin/check_p2t2c.sh: failed; see /tmp/p2t2c-upgrade-check.log"
  fi
