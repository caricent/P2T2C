...
```bash
cp -R P2T2C_CN/. your-project/
cd your-project
cp project_config.example.yaml project_config.yaml
```

编辑 `project_config.yaml`，填写项目名称、描述、语言和技术栈。
...
已经使用当前外壳版本的项目也可以使用本地 Makefile alias。
...
| `README.md` | 人类入口 |
| `AGENTS.md` | AI 操作入口 |
| `P2T2C_LICENSE.md` | 单独复制发行根时保留的 MIT license notice |
| `project_config.example.yaml` | 项目配置模板 |
| `CHECKSUMS.sha256` | release 文件校验和 |
...
| `sdd/templates/` | Spec、Plan、Tasks 模板 |
| `specs/` | 功能执行文档 |
| `templates/` | 可复用 P2T2C 产物模板 |
| `prompts/` | AI agent 阶段 prompt |
| `migrations/p2t2c/` | 模板迁移说明 |
